...
# 发送 0 字节的数据是什么效果？
send 或 recv 函数返回 0 表示对端关闭了连接。可能认为如果发送 0 字节数据，那么 send 函数也会返回 0，对端会 recv 到 0 字节的数据，事实是这样的吗？
...
因此，server 端也会一直没有输出，如果用的是 gdb 启动 server，此时中断下来会发现，server 端由于没有数据会一直阻塞在 recv 函数调用处。
...
然而，这里的代码示例仅仅用于实验性讨论，send 一个 0 字节数据是没有任何意义的，在实际开发时，避免写出可能调用 send 函数发送 0 字节的代码。

# connect 函数在阻塞和非阻塞模式下的行为
当 socket 使用阻塞模式时， connect 函数会一直到有明确的结果才会返回（或连接成功或连接失败），如果服务器地址“较远”或者网络状况不好，连接速度比较慢，程序可能会在 connect 函数处阻塞好一会儿（如两三秒之久）。虽然这一般也不会对依赖于网络通信的程序造成什么影响，但在实际项目中，我们一般倾向使用所谓的异步 connect 技术，或者叫非阻塞 connect。这个流程一般有如下步骤：
...
- 在 Windows 系统上，一个 socket 没有建立连接之前，使用 select 函数检测其是否可写，能得到正确的结果（不可写），连接成功后检测，会变为可写。所以，上述介绍的异步 connect 写法流程在 Windows 系统上是没有问题的。

- 在 Linux 系统上一个 socket 没有建立连接之前，用 select 函数检测其是否可写，你也会得到可写的结果，所以上述流程并不适用于 Linux 系统。Linux 系统上正确的做法是，connect 之后，不仅要用 select 检测可写，还要调用 getsockopt 检测此时 socket 是否出错，通过错误码来检测确定是否连接上，错误码为 0 表示连接上，反之则表示未连接上。完整代码如下：
...
Linux 操作系统上，步骤 3 中，可以使用 poll 函数代替 select 函数来判断 socket 是否可写；在 Windows 平台上可以使用 WSAEventSelect 或 WSAAsyncSelect 函数判断连接是否成功。
...
上述代码逻辑其实是有问题的，因为当你调用完 ioctlsocket 或 ioctl 函数在调用 recv 或 read 函数之前，可能接收缓冲区又新增了一段数据，导致实际调用 recv 可以收到的数据长度大于 bytesToRecv，因此建议不要基于这样的认知去做一些逻辑上的假设，以免编写出错误的逻辑来。
